...
export class SearchEngine {
  private docIndex: FlexSearch.Document<Document, string[]>;
  private documents: Map<string, Document>;

  constructor() {
    this.documents = new Map();

    this.docIndex = new FlexSearch.Document({
      document: {
        id: 'id',
        index: ['title', 'content'],
  ...
  async search(query: SearchQuery): Promise<SearchResult[]> {
    ...
    for (const result of results) {
      ...
      for (const item of result.result) {
        const docId = typeof item === 'string' ? item : (item as any).id;
        const doc = this.documents.get(docId);
        if (!doc) continue;

        // Apply type filter
        if (type !== 'all') {
          if (type === 'docs' && doc.type !== 'markdown') continue;
          if (type === 'code' && doc.type === 'markdown') continue;
        }
   ...
   */
  private findHighlights(content: string, searchTerm: string, limit: number = 5): string[] {
   ...
   * Calculate relevance score
   */
  private calculateScore(doc: Document, searchTerm: string): number {
    let score = 0;
   ...
   */
  private hasAnyTag(docTags: string[] | undefined, filterTags: string[]): boolean {
