...
class FeatureMapper {
     ...
     */
    parseComponents(componentsStr) {
        if (!componentsStr || componentsStr === 'N/A')
            return [];
     ...
     * Parse tier access
     */
    parseTierAccess(tierStr) {
        const lower = tierStr.toLowerCase();
        if (lower.includes('gladiator'))
            return 'gladiator';
     ...
     */
    determineRouteType(path, typeHint) {
     ...
     */
    mapFeaturesToTables(features, schemaDoc) {
     ...
     */
    mapFeaturesToAPIs(features, apiDoc) {
